...
module.exports = React.createClass({
  ...
  , enableDateFilter : function() {
    ...
    if (typeof this.state.start_date == 'undefined') this.state.start_date = Moment();
    if (typeof this.state.end_date == 'undefined') this.state.end_date = Moment();
  ...
  , render: function () {
    ...
    var Transactions = this.state.Transactions.filter(function(Transaction){
              if (this.state.date_filter) {
                
                var tr_date = new Date(Transaction[1]);
                var start_date = new Date(this.state.start_date.format('LL'));
                var end_date = new Date(this.state.end_date.format('LL'));
                
                if ((start_date.getTime() <= tr_date.getTime()) && (end_date.getTime() >= tr_date.getTime())) {
                  return true;  
                } else {
                  return false;
                }
      ...
      <div>
        <header>
            ...
            <SplitButton className="userInfo" title={<Glyphicon glyph='user' className = 'glyph-selected' />} pullRight>
              ...
              <MenuItem eventKey={3} href='#login'>Logout </MenuItem>
              <MenuItem divider />
        ...
        {this.state.date_filter?
          <Well>
            ...
            <p>Displaying transactions from: {Moment(this.state.start_date).format('LL')} to: {Moment(this.state.end_date).format('LL')}.
            <Button bsStyle='success' onClick={this.changeDateFilter}>Change</Button></p>
            <p><Button bsStyle='primary' onClick={this.disableDateFilter}>Remove filters</Button></p>
          </Well>
          : 
          <Well>
            ...
            {this.state.show_date_filter?
              <p>
                <Row>
                  ...
                  <Col xs={3}>
                    <Button bsStyle='success' onClick={this.enableDateFilter}>Apply filter</Button>
                    <Button bsStyle='danger' onClick={this.hideDateFilter}>Cancel</Button>
                  </Col>
                </Row>
              </p>
            :
              <p><Button bsStyle='success' onClick={this.showDateFilter}>Filter by date</Button></p>
            }
          </Well>
        }
